...
**Status:** phases 0–4 complete · **Date:** 2026-09-12 (plan v2: 2026-09-09) · **Target:** A-level Math / Chemistry / Biology

> **Progress:** Phases 0 (foundation), 1 (data + syllabus), 2 (FSRS +
> scheduling), 3 (LLM provider & generation) and 4 (Gmail + Calendar + Tasks) are
> built and verified. Phase 3 reaches the interface: the Generate panel runs a
> pipeline, the review gate stands between the model and the database, and a
> generated quiz can be taken in the Review section. Phase 4 reaches it too:
> Settings → Google can connect an account, test both services and index the
> calendar, and twelve Google tools are now declared runnable, five of them behind
> the review gate. Two things are **not** proven, both for want of credentials: no
> phase-3 run has met a real model, and no phase-4 call has met the real Google
> API. See the notes under phases 3 and 4, [`CHANGELOG.md`](CHANGELOG.md) for what
...
| **calendar** | `calendar_list_calendars`, `calendar_list_events`, `calendar_get_event`, `calendar_create_event`, `calendar_update_event`, `calendar_list_tasklists`, `calendar_list_tasks`, `calendar_create_task`, `calendar_update_task` |

**As built (12, not the 13 listed here originally):** there is no
`calendar_delete_event`. The reference implementation has no delete tool either,
and its absence is what makes every mutating Google tool terminal — nothing in the
set can be undone from inside the app, so all five writes sit behind the review
gate.
...
- **As built:** the shared core is `mcp/lib/google-client.mjs` (auth + REST + error
  shape + payload scrub), and the consent flow lives beside it in
  `mcp/lib/google-oauth.mjs` so the terminal script and the in-app button share
  one implementation.
- **As built:** friendly calendar names live in `<userData>/calendars.json`,
  refreshed from `calendar_list_calendars` — not `safe/calendars.json`, which is
  gitignored, never shipped, and in the reference was generated by a script that
  no longer exists.
- All external payloads pass through `sanitizeObject()` before logging.
- Tool calls log through `shared/logger.mjs`; the app also routes each server's
  stderr into the Dev panel under the `mcp` source.
...
### Phase 4 — Gmail + Calendar MCP (Days 18–22) ✅ **complete**
- `mcp/lib/` shared core (Google REST + OAuth) + `mcp/gmail` + `mcp/calendar` stdio servers.
- `shared/tool-manifest.js`, a hand-rolled MCP **host** in the main process, and
  `services/google/` (gateway + calendar index).
- Settings → **Google** section (connect, test, refresh calendars, source annotations).
- `scripts/gmail-auth.mjs` for the terminal path; the same consent flow in-app.

**Built as:** the two servers use the official MCP SDK, but the **app speaks
JSON-RPC to them by hand** — the SDK is 16 transitive packages (Express, Hono,
`jose`, `ajv`) and would have had to be bundled into the main process for four
method names. Google access is plain `fetch`; neither `googleapis` nor
`google-auth-library` is a dependency. The reference repo never built an MCP host
at all: it spawned the servers as opaque supervised services and duplicated the
Google logic inside its own main process.

See `CHANGELOG.md` `[0.0.2-3]` for the full list. Three things worth knowing:

- **Nothing executed a pipeline tool before this phase.** `executorFor()` was
  consulted only for classification and warnings. Phase 4 adds
  `services/generation/dispatch.ts`, which runs the `pre` Google reads into a
  `{{toolContext}}` prompt block and the `commit` writes as a **separate step
  after** the database commit — so a Google call never sits inside a savepoint and
  a failed email cannot roll back saved cards.
- **Every mutating Google tool is terminal**, because nothing in the set can be
  undone from inside the app (there is no delete or unsend tool). This corrected
  `calendar_create_task`, which writes to Google but was marked reversible.
- **`mcp/` had to be added to `extraResources`**, since the servers need their own
  `node_modules` at run time. Packaging is no longer exclusively phase 7's
  concern, though the bundled runtime dependencies still are.

**Unverified:** no live Google call has been made, and the renderer still has no
test infrastructure, so the new Settings controls are hand-verified only.
...
| Adopt the two MCP servers as-is | The **servers** use the official SDK; the **app** speaks JSON-RPC to them by hand | The SDK is 16 transitive packages (Express, Hono, `jose`, `ajv`) and would have become a dependency of the main process for four method names. The reference never built an MCP host at all — it spawned the servers as opaque supervised services and duplicated the Google logic in its own main process. |
| `googleapis` + `google-auth-library` | Plain `fetch` against the Google REST APIs | Both would have had to ship inside each server's `node_modules`. The reference's own OAuth flow already uses raw `fetch`; Node has a global `fetch`. |
| `safe/calendars.json` for friendly names | `<userData>/calendars.json`, refreshed from `calendar_list_calendars` | `safe/` is gitignored and never shipped, and the reference's generator script does not exist — so its map only ever resolved `"primary"`. |
| 13 Google tools, incl. `calendar_delete_event` | 12 — the reference has no delete tool | Writing delete from scratch was not worth it, and its absence is precisely what makes all five mutating tools terminal. |
| `scripts/gmail-auth.mjs` is the only auth path | The script **and** an in-app **Connect Google** button | `scripts/` is not shipped in `extraResources`, so a packaged install would have had no way to obtain a token. |
| — (unspecified) | `mcp/` added to `extraResources` | The MCP servers need their own `node_modules` at run time, so packaging stopped being purely phase 7's problem. The bundled *runtime* dependencies (`ts-fsrs`, `papaparse`, `mammoth`, `pdfjs-dist`) still are. |
| — (unspecified) | Pipeline steps gained an optional `args` object | A model call gets its input from the rendered prompt; a tool that is **not** a model call had nowhere else to get it. Values support `{{placeholder}}` interpolation, recursing into arrays and objects. |
...
- **Prompt-injection:** all external data (email bodies, event descriptions, imported syllabus) passes
  ...
  **As built (4J):** the object path is live too. `sanitizeObject()` walks a
  decoded payload recursively, applies the same baseline to every string, and
  strips invisible and bidirectional control characters — which are unreadable in
  a log while still reaching a model intact. It runs inside
  `mcp/lib/google-client.mjs` before a payload is returned or logged, and again in
  `services/google/gateway.ts`, so both boundaries are covered. The rule set now
  exists in JavaScript as well as TypeScript (`shared/sanitize-core.mjs`), because
  the MCP servers cannot import the TypeScript module; a parity test asserts the
